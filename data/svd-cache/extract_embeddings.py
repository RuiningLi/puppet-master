...
if __name__ == "__main__":
    ...
    for dataset_root, category, obj, action in tqdm(all_jobs[args.job_id::args.num_jobs]):
        ...
        if len(all_image_paths) == 0:
            continue
        num_computed = len([p for p in glob(osp.join(args.save_dir, category, obj, action, "*.pt")) if "latents" not in p])
        if num_computed == len(all_image_paths):
            continue
        
        for i in range(0, len(all_image_paths), batch_size):
            image_paths = all_image_paths[i:i+batch_size]
            images = [convert_rgba_to_rgb_with_white_bg(image_path).resize((256, 256), resample=Image.LANCZOS) for image_path in image_paths]
            with torch.no_grad():
                embeddings = pipe._encode_image(images, device=device, num_videos_per_prompt=1, do_classifier_free_guidance=False)
                for j, image_path in enumerate(image_paths):
                    embedding = embeddings[j]
                    embedding_path = osp.join(args.save_dir, category, obj, action, image_path.split("/")[-1].replace(".png", "_embedding.pt"))
                    os.makedirs(osp.dirname(embedding_path), exist_ok=True)
