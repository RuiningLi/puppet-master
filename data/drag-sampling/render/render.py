...
def shade(
    ...
    if 'kd_ks_normal' in material:
        ...
        if 'ka' in material:
            ka = material['ka'].sample(gb_texc, gb_texc_deriv)[..., 0:3] # skip alpha``
    ...
    if bsdf == 'pbr':
        ...
        if isinstance(lgt, light.EnvironmentLight):
            ...
            shaded_col = lgt.shade(gb_pos, gb_normal, ka, kd, ks, view_pos, specular=False)
        else:
            assert False, "Invalid light type"
    elif bsdf == 'normal':
...
def render_layer(
    ...
    if mesh.v_tex is None:
        assert mesh.t_tex_idx is None
        mesh.v_tex = torch.zeros_like(mesh.v_pos[..., :2]) + 0.5
        mesh.t_tex_idx = mesh.t_pos_idx
