...
######################################################################################
@torch.no_grad()
def load_mtl(fn, clear_ks=True, device="cuda"):
    import re
    ...
    for mat in materials:
        ...
        if "map_ka" in mat:
            mat["ka"] = texture.load_texture2D(os.path.join(mtl_path, mat["map_ka"]))
        elif "ka" in mat:
...
def merge_materials(materials, texcoords, tfaces, mfaces):
    ...
    thresh = 65536
    if full_res[1] > thresh:
        scale = thresh / full_res[1]
        full_res[1] = thresh
        max_res[1] = (max_res[1] * scale).astype(np.int)
    ...
    for fi in range(len(tfaces)):
        ...
        for vi in range(3):
            ...
            if not (matIdx in new_tverts[ti]): # create new vertex
                if ti >= 0:
                    new_tverts_data.append([(matIdx + float(texcoords[ti][0]) % 1) / s_coeff[1], float(texcoords[ti][1]) % 1 / s_coeff[0]]) # Offset texture coodrinate (x direction) by material id & scale to local space. Note, texcoords are (u,v) but texture is stored (w,h) so the indexes swap here
                else:
                    new_tverts_data.append([(matIdx + 0.5) / s_coeff[1], 0.5 / s_coeff[0]])
