...
class DirectionalLight(torch.nn.Module):
    def __init__(self, color, direction, min_kd=0.4):
        super(DirectionalLight, self).__init__()
        self.mtx = None
        color = color / torch.max(color)
        direction = util.safe_normalize(direction)
        self.color = torch.nn.Parameter(color.clone().detach(), requires_grad=True)
        self.register_parameter('color', self.color)

        self.direction = torch.nn.Parameter(direction.clone().detach(), requires_grad=True)
        self.register_parameter('direction', self.direction)
        self.min_kd = min_kd
    ...
    def shade(self, gb_pos, gb_normal, ka, kd, ks, view_pos, specular=True, alpha=1):
        ...
        kd = kd * self.color * (torch.clamp(util.dot(gb_normal, self.direction), min=0) * (1 - self.min_kd) + self.min_kd)

        if specular:
            ks = ks * self.color * torch.pow(torch.clamp(util.dot(self.direction, reflvec), min=0.0), alpha)
        else:
