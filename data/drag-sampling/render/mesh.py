...
class Mesh:
    ...
    def first_n(self, n):
        return Mesh(self.v_pos[:n], self.t_pos_idx, self.v_nrm[:n], self.t_nrm_idx, self.v_tex[:n], self.t_tex_idx, self.v_tng[:n], self.t_tng_idx, self.material)
    ...
    def extend(self, N: int):
        ...
        uvs = self.v_tex.repeat(N, 1, 1) if self.v_tex is not None else None
        uv_idex = self.t_tex_idx
        mat = self.material

        return make_mesh(verts, faces, uvs, uv_idex, mat)
...
def load_mesh(filename, mtl_override=None):
    ...
    try:
        if ext == ".obj":
            return obj.load_obj(filename, clear_ks=False, mtl_override=mtl_override)
...
def auto_normals(imesh):
    ...
    v_nrm = v_nrm.scatter_add(1, i0[None, :, None].repeat(batch_size, 1, 3), face_normals)
    v_nrm = v_nrm.scatter_add(1, i1[None, :, None].repeat(batch_size, 1, 3), face_normals)
    v_nrm = v_nrm.scatter_add(1, i2[None, :, None].repeat(batch_size, 1, 3), face_normals)
...
def compute_tangents(imesh):
    ...
    for i in range(0,3):
        ...
        # tex[i] = imesh.v_tex[:, imesh.t_tex_idx[0, :, i]] if imesh.v_tex is not None else None
        tex[i] = None
    ...
    uve1 = tex[1] - tex[0] if tex[0] is not None else None  # Shape: (B, F, 2)
    uve2 = tex[2] - tex[0] if tex[0] is not None else None  # Shape: (B, F, 2)
    pe1  = pos[1] - pos[0]  # Shape: (B, F, 3)
    pe2  = pos[2] - pos[0]  # Shape: (B, F, 3)
    
    nom   = pe1 * uve2[..., 1:2] - pe2 * uve1[..., 1:2] if uve1 is not None else None  # Shape: (B, F, 3)
    denom = uve1[..., 0:1] * uve2[..., 1:2] - uve1[..., 1:2] * uve2[..., 0:1] if uve2 is not None else None  # Shape: (B, F, 1)
    
    # Avoid division by zero for degenerated texture coordinates
    tang = nom / torch.where(denom > 0.0, torch.clamp(denom, min=1e-6), torch.clamp(denom, max=-1e-6)) if denom is not None else None  # Shape: (B, F, 3)
    ...
    for i in range(0,3):
        ...
        if tang is not None:
            ...
            tangents = tangents.scatter_add(1, idx, tang)
            tansum = tansum.scatter_add(1, idx, torch.ones_like(tang))
            tangents = tangents / torch.clamp(tansum, min=1e-6)
...
######################################################################################
# Convert a single mesh to a batched mesh.
######################################################################################
def batch_mesh(mesh):
    assert (len(mesh.v_pos.shape) == 2 
            and len(mesh.t_pos_idx.shape) == 2 
            and (mesh.v_tex is None or len(mesh.v_tex.shape) == 2)
            and (mesh.t_tex_idx is None or len(mesh.t_tex_idx.shape) == 2)), "All components must be unbatched."
    new_mesh = mesh.clone()
    new_mesh.v_pos = new_mesh.v_pos[None, ...]
    new_mesh.t_pos_idx = new_mesh.t_pos_idx[None, ...]
    if new_mesh.v_tex is not None:
        new_mesh.v_tex = new_mesh.v_tex[None, ...]
    if new_mesh.t_tex_idx is not None:
        new_mesh.t_tex_idx = new_mesh.t_tex_idx[None, ...]
    if new_mesh.v_nrm is not None:
        new_mesh.v_nrm = new_mesh.v_nrm[None, ...]
    if new_mesh.t_nrm_idx is not None:
        new_mesh.t_nrm_idx = new_mesh.t_nrm_idx[None, ...]
    if new_mesh.v_tng is not None:
        new_mesh.v_tng = new_mesh.v_tng[None, ...]
    if new_mesh.t_tng_idx is not None:
        new_mesh.t_tng_idx = new_mesh.t_tng_idx[None, ...]
    return new_mesh
